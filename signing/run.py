...
if os.path.splitext(os.path.basename(__file__))[0] == 'cProfile':
    if os.environ.get('SIGNING_PATH'):
        ROOT = os.environ['SIGNING_PATH']
    else:
        print 'When using cProfile you must set $SIGNING_PATH'
        sys.exit(2)
...
# setting up logging
ini_file = '/etc/mozilla-services/signing/production.ini'
ini_file = os.environ.get('SIGNING_INI', ini_file)
try:
    fileConfig(ini_file)
